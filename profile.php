...
if ($_POST) {
    ...
    if ($nombre && $email) {
        try {
            $sql = "UPDATE users SET nombre = ?, email = ?, telefono = ?, birth_date = ?, address = ?, turno_id = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->execute([$nombre, $email, $telefono, $birth_date, $address, $turno_id, $currentUser['id']]);
...
// Obtener datos completos del usuario con información del turno
try {
                        ...
                        <div class="col col-6">
                            <div class="form-group">
                                ...
                                <input 
                                    ...
                                    id="num_empleado" 
                                    class="form-input" 
                                    value="<?php echo htmlspecialchars($userData['num_empleado'] ?? ''); ?>"
                                    disabled
                                >
                            </div>
                        </div>
                        <div class="col col-6">
                            <div class="form-group">
                                ...
                                <input 
                                    ...
                                    id="rol" 
                                    class="form-input" 
                                    value="<?php echo ucfirst($userData['rol'] ?? ''); ?>"
                                    disabled
                                >
                            </div>
                        </div>
                                ...
                                <select id="turno_id" name="turno_id" class="form-select">
                                    ...
                                    <?php foreach ($turnos as $turno): ?>
                                        <option value="<?php echo $turno['id']; ?>" 
                                                <?php echo (($userData['turno_id'] ?? '') == $turno['id']) ? 'selected' : ''; ?>>
                ...
                <div class="row">
                    <div class="col col-6">
                        <p><strong>Estado:</strong> 
                            <span class="badge badge-<?php echo ($userData['status'] ?? '') === 'activo' ? 'success' : 'secondary'; ?>">
                                <?php echo ucfirst($userData['status'] ?? 'inactivo'); ?>
                            </span>
                        </p>
                        <p><strong>Turno Actual:</strong>
                            <?php if ($userData['nombre_turno']): ?>
                                <span class="badge badge-info">
                                    <?php echo htmlspecialchars($userData['nombre_turno']); ?>
                                </span>
                            <?php else: ?>
                                <span class="badge badge-secondary">Sin turno asignado</span>
                            <?php endif; ?>
                        </p>
                        <p><strong>Miembro desde:</strong> <?php echo date('d/m/Y', strtotime($userData['created_at'] ?? '')); ?></p>
                    </div>
                    <div class="col col-6">
                        <p><strong>Última actualización:</strong> <?php echo date('d/m/Y H:i', strtotime($userData['updated_at'] ?? '')); ?></p>
                    </div>
                </div>
            </div>
        </div>
